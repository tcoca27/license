...
def hoop_dist_attackers(attack):
    res = {}
...
def hung_alg(attack, defence):
    att_dist = hoop_dist_attackers(attack)
...
# Four corners of the court in destination image (start top-left corner and go clock wise)
...
for im_path in images_path:
    ...
    f = open(frame_path + '\\' + im_name + '_paint.txt', 'r')
    line = f.readline()
    f.close()
    tokens = line.split(" ")
    ...
    # draw players
    player_positions = glob.glob(frame_path + '\\' + im_name + '\*.txt')
    ...
    if len(prev_att) == 0 and len(prev_def) == 0:
        ...
        best_att, max_d = hung_alg(res_att, res_def)
...
def return_to_3d(twod, image_name, pts_src):
    im = cv2.imread(frame_path + '\\' + image_name + '.jpg')
...
for imgs in res[-5:]:
    return_to_3d(imgs[0], imgs[1], imgs[2])
    fig = plt.figure(figsize=(15, 15))
    fig.add_subplot(1, 3, 1)
    plt.imshow(cv2.cvtColor(imgs[0], cv2.COLOR_BGR2RGB))
    plt.title('Detected')
    fig.add_subplot(1, 3, 2)
    im = cv2.imread(frame_path + '\\' + imgs[1] + '.jpg')
    plt.imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
    plt.title('OG')
    plt.show()